...
// ── CHAPTER FINDER ──
// Pulls live from /data/chapters.json and sorts by distance if geolocation available
...
function initChapterFinder() {
  ...
  function renderChapters(list) {
    ...
    resultsEl.innerHTML = list.map(function(c) {
      var distLabel = c._distMiles != null
        ? '<span style="font-size:12px;color:var(--teal);font-weight:600;">' + c._distMiles + ' miles away</span>'
        : '';
      var focusText = Array.isArray(c.focus) ? c.focus.join(', ') : (c.focus || '');
      var badgeLabel = c.status === 'active' ? 'Active' : c.status === 'forming' ? 'Forming' : 'Just started';
      return '<div class="chapter-card">' +
        '<div class="chapter-info">' +
          '<h4>' + c.name + ', ' + c.state + '</h4>' +
          '<p>' + c.members + ' member' + (c.members !== 1 ? 's' : '') + ' &middot; ' + focusText + '</p>' +
          distLabel +
        '</div>' +
        '<div style="display:flex;gap:0.75rem;align-items:center;flex-shrink:0">' +
          '<span class="chapter-badge badge-' + c.status + '">' + badgeLabel + '</span>' +
  ...
  function filter() {
    ...
    if (query) list = list.filter(function(c) {
      return c.name.toLowerCase().includes(query) || c.state.toLowerCase().includes(query);
    });
    if (status !== 'all') list = list.filter(function(c) { return c.status === status; });
    renderChapters(list);
  }

  resultsEl.innerHTML = '<div class="no-results"><p>Loading chapters\u2026</p></div>';
  ...
  fetch('/data/chapters.json')
        ...
        navigator.geolocation.getCurrentPosition(
          function(pos) {
            allChapters.forEach(function(c) {
              var km = haversineKm(pos.coords.latitude, pos.coords.longitude, c.lat, c.lng);
              c._distMiles = kmToMiles(km);
              c._distKm = km;
            });
            allChapters.sort(function(a, b) { return a._distKm - b._distKm; });
            filter();
          },
          function() { filter(); }
        );
    ...
    .catch(function() {
      resultsEl.innerHTML = '<div class="no-results"><p>Could not load chapters. Please try again.</p></div>';
    });
